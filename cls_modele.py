...
class IncompressibleModels():
    ...
    params: dict of modele parameters. Depending on the model choice, dict keys 
    are different. 
    The params should at least contain the model function name under the key 
    'model' (e.g.: {'model':'model_rivlin'}) to avoid class error
    '''
    def __init__(self, values, params):
        self.C = ct.StrainTensor(values) 
        #creates the object C - see cls_tensors.py 
        #for further detail on the methods of the class
        self.model = getattr(self, params['model']) #calls the adequate method
        self.params = params
    ...
    def modele_rivlin(self):
        ...
        Parameters
        ----------
        C0, C1 : model parameters specified when calling the class in the dict 
        params entry
        ...
        try: 
            ...
            return ('Error : class function requires two parameters 
                (C0 and C1) to run')
    
    def St_Venant(self, E = None):
        """
        Returns the modeled dual stress computed from a hyperelastic anisotropic
        kelvin approach, using the Hencky deformation measure.
        
        Parameters
        ----------
        params: defined when calling the class. Dict keys are 
        {'lh', 'l1', 'l2', 'l3', 'l4', 'l5', 'proj', 'penal'}. 
        'li' are the kelvin modules, 'proj' contains the kelvin eigentensors 
        and 'penal' is defined to deals with incompressibility
        
        Returns
        -------
        S: the modeled dual stress tensor written in mandel notation. 
        Shape (6, n) : n is the number of tensors corresponding with the number 
        of deformation tensors in input
        """
        lh, l1, l2, l3, l4, l5 , a = (self.params['lh'], 
                                      self.params['l1'], 
                                      self.params['l2'], 
                                      self.params['l3'], 
                                      self.params['l4'], 
                                      self.params['l5'], 
                                      self.params['a'])
        # Kelvin modulus and power order of the law
        proj = self.params['proj'] # kelvin projectors
        k = self.params['penal'] # penalty coefficient for incompressibility
